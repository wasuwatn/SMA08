...
// The portal is normally served by this same hub, so the app's own origin is
// the right base. VITE_PORTAL_BASE overrides it when the portal lives on
// another host (e.g. the HTTPS LIFF deployment) while staff use a local app.
const claimUrl = (token) =>
  `${(import.meta.env.VITE_PORTAL_BASE || window.location.origin).replace(/\/$/, '')}/customer.html?claim=${token}`;
