// KOTEA launcher: starts the API/server (which also serves the built client)
// and opens the Main app in the default browser once it's ready. If the
// server is already running (e.g. started by the POS or Expense launcher),
// this just opens the page instead of starting a second server.
...
const logFile = path.join(baseDir, 'kotea.log');
...
log(`Starting KOTEA from ${baseDir}`);
