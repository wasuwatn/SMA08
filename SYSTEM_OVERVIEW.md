...
> อัปเดตล่าสุด: 2026-07-10 · เอกสารนี้สรุปภาพรวมทุกส่วนของระบบ ทั้ง repo นี้ (SMA08)
...
### Deploy branch strategy — `main` vs `production`

ตั้งแต่ 2026-07 มี 2 branch แยกหน้าที่กันชัดเจน เพื่อกัน**งาน Mother ที่ยังพัฒนาไม่เสร็จ
ไปกระทบระบบที่ลูกค้าใช้งานจริงอยู่** (customer.html + coffee-pos-buddy ต้องพึ่ง hub
ตัวเดียวกันกับที่ Mother ใช้อยู่ เพราะเป็น Express process เดียวที่เสิร์ฟ API ให้ทุกแอป):

| Branch | ใช้ทำอะไร | Auto-deploy ไปที่ไหน |
|---|---|---|
| `main` | พัฒนา Mother (dashboard, รายงาน, LINE automation ฯลฯ) ต่อเนื่อง | ไม่ deploy อัตโนมัติ |
| `production` | โค้ดที่ผ่านการทดสอบแล้วว่าไม่กระทบ checkout/points/customer — เป็นโค้ดที่ลูกค้าใช้จริง | **Render** (hub) + **Vercel** (`sma-08-ashen.vercel.app`, customer.html) — ตั้ง Production Branch เป็น `production` ทั้งสองที่แล้ว |

**กติกา**: งานใหม่ทั้งหมด push เข้า `main` ตามปกติ **ห้าม merge เข้า `production`
จนกว่าจะตรวจแล้วว่าไม่กระทบ** `/api/checkout/*`, `/api/customer/*`, `/api/points/*`,
`/api/redemption/*` — พอพร้อมค่อย merge `main` → `production` ทีเดียว ระบบจะ redeploy
ให้อัตโนมัติทั้ง Render และ Vercel

**หมายเหตุ**: coffee-pos-buddy เป็นคนละ repo ไม่ได้ใช้ไฟล์ร่วมกับ Mother เลย จึงไม่มี
ความเสี่ยงแบบนี้ — deploy ตามปกติของมันเองได้เลยไม่ต้องผูกกับ branch strategy นี้
