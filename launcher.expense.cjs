// KOTEA Expense launcher: starts the API/server (which also serves the
// built client) and opens the Expense page in the default browser once it's
// ready. If the server is already running (e.g. started by the Main or POS
...
log(`Starting KOTEA Expense from ${baseDir}`);
